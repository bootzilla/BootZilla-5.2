...
set devtools=7z.exe a -t7z -m0=LZMA -mmt=on -mx9 -md=64m -mfb=64 -ms=on "devtools.7z" bootzilla.ibb selector.cmd createdirs.cmd msvcr71.dll 7zsfxmkr31.cmd sfxpatch2.cmd BuildPackage.cmd devsetup.cmd 7z.exe 7z.dll 7zS.sfx
:: Almost forgot that I needed to add a 7zip-compressed version of the frelease, for v4.2 users to upgrade to.
set frup=7z.exe a -t7z -m0=LZMA -mmt=on -mx9 -md=64m -mfb=64 -ms=on "latest.7z" bootzilla.ibb imgburn.exe jkdefragcmd.exe qdefrag.cmd bzar5.7z selector.cmd bzmultitool.cmd createdirs.cmd 7zsfxmkr31.cmd sfxpatch2.cmd 7zS.sfx devsetup.cmd msvcr71.dll bz_mal.cmd bz_utils.cmd bz_apps.cmd 7z.exe 7z.dll index.html bzstyles.css bz.ico logger.exe nocrlf.exe wget.exe ImgBurnLicenseAgreement.rtf license.txt isolinux.bin Readme.txt decompress.cmd changelog.txt BZUpdate.cmd sanitize.cmd BuildPackage.cmd updates\ updates\*
::
cls
color 71
echo.
